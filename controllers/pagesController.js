...
exports.home = function (params){
  return async function(req, res, next) {
    ...
    const recentlyadded = await Matches.findAll({ order: [ [ 'updatedAt', 'DESC' ]], limit: 10 });
    const valorantmatches = await Matches.findAll({ where: {game_id: 1},order: [ [ 'updatedAt', 'DESC' ]], limit: 10})
    const r6matches = await Matches.findAll({ where: {game_id: 2},order: [ [ 'updatedAt', 'DESC' ]], limit: 10})
...
exports.upload = function (params){
  return async function(req, res, next) {
    ...
    res.render("pages/upload", { cloudinary: params.cloudinary, gametags: gametags, match: match, user: user})
  };
};
...
exports.profile = function (params){
  return async function(req, res, next) {
    ...
    if (req.user){
      var user = await Users.findOne({ where: {id: req.user.id}, include: {model: Matches} });
    }
    res.render("pages/profile", { cloudinary: params.cloudinary, user: user})
  };
};
